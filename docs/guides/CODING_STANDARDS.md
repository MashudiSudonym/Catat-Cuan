...
|------|------------|---------|
| **Entities** | `snake_case` + `_entity.dart` | `transaction_entity.dart` |
| **Use Cases** | `verb_noun` + `_usecase.dart` | `add_transaction_usecase.dart` |
| **Repository Interfaces** | `noun_repository.dart` | `transaction_read_repository.dart` |
| **Repository Implementations** | `noun_repository_impl.dart` | `transaction_read_repository_impl.dart` |
| **Models** | `noun_model.dart` | `transaction_model.dart` |
| **Providers** | `noun_provider.dart` | `transaction_list_provider.dart` |
| **Screens** | `noun_screen.dart` | `transaction_form_screen.dart` |
| **Widgets** | `noun_widget.dart` | `transaction_card_widget.dart` |
| **Controllers** | `noun_controller.dart` | `transaction_form_controller.dart` |
| **States** | `noun_state.dart` | `transaction_form_state.dart` |
...
// lib/presentation/providers/transaction/transaction_list_provider.dart
@riverpod
...
// ❌ WRONG
// lib/domain/entities/Transaction.dart  // Should be snake_case
// lib/domain/usecases/AddTransaction.dart  // Missing _usecase suffix
// lib/domain/repositories/TransactionRepo.dart  // Abbreviations not recommended
```
...
import 'package:catat_cuan/domain/usecases/add_transaction_usecase.dart';
...
final result = await useCase.execute(TransactionEntity(amount: state.nominal, type: state.type, dateTime: DateTime(state.date.year, state.date.month, state.date.day, state.time.hour, state.time.minute), categoryId: state.categoryId, note: state.note, createdAt: DateTime.now()));
```
...
```dart
/// Use case for adding a new transaction to the database.
///
/// This use case validates the transaction entity before adding it.
...
/// ```
class AddTransactionUseCase {
  /// Executes the use case with the given [transaction].
  ///
  /// Returns [Right] with the added transaction including its ID,
...
```dart
/// Provider for managing transaction list state.
///
...
```dart
/// Loads transactions from the database with optional pagination.
///
/// Parameters:
...
///
/// Returns a paginated result containing the transactions and
/// whether more data is available.
///
/// Throws [DatabaseException] if the query fails.
Future<PaginatedResult<TransactionEntity>> loadTransactions({
  int limit = 20,
  int offset = 0,
...
```dart
// Calculate the monthly aggregation by grouping transactions by month
// and summing amounts per category
...
Future<Either<Failure, TransactionEntity>> addTransaction(
  ...
  try {
    final id = await _database.insert(transaction);
...
Future<TransactionEntity> addTransaction(
  ...
  try {
    ...
    throw Exception('Failed to add transaction'); // Don't throw!
  }
}
```
...
final state = ref.watch(transactionListProvider);
return TransactionList(transactions: state.value!); // Crashes if error!
```
...
void main() {
  ...
  group('AddTransactionUseCase', () {
    ...
    test('should add transaction successfully', () async {
      ...
      final expected = transaction.copyWith(id: 1);
...
|------|-------------|---------|
| `feat` | New feature | `feat(transaction): add pagination support` |
...
| `test` | Test changes | `test(usecases): add tests for add transaction` |
...
```bash
# Feature
git commit -m "feat(transaction): add infinite scroll pagination
...
# Refactor
git commit -m "refactor(repositories): segregate category repositories

- Split category repository into 4 focused interfaces:
  - CategoryReadRepository
  - CategoryWriteRepository
  - CategoryManagementRepository
  - CategorySeedingRepository
